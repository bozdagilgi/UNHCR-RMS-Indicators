...
# Impact	2.2 Proportion of PoCs residing in physically safe and secure settlements with access to basic facilities
# Impact	2.3 Proportion of PoC with access to health services
# Impact	3.2a Proportion of PoC enrolled in primary education
# Impact	3.2b Proportion of PoC enrolled in secondary education
# Impact	3.3 Proportion of PoC feeling safe walking alone in their neighborhood (related SDG 16.1.4).
# Outcome	1.2 Proportion of children under 5 years of age whose births have been registered with a civil authority. [SDG 16.9.1 - Tier 1]
# Outcome	1.3 Proportion of PoC with legally recognized identity documents or credentials [GCR 4.2.2].
# Outcome	8.2 Proportion of PoC with primary reliance on clean (cooking) fuels and technology [SDG 7.1.2 Tier 1]
# Outcome	13.1. Proportion of PoC with an account at a bank or other financial institution or with a mobile-money-service provider [SDG 8.10.2 Tier 1].
# Outcome	13.2. Proportion of PoC who self-report positive changes in their income compared to previous year.
...
main <- main %>%
 #Only apartment and house
  ...
  mutate(dwe03_cat=case_when(  (DWE03_num==8 |DWE03_num==9 | DWE03_num==10 | DWE03_num==11 |
                               DWE03_num==12 | DWE03_num==13 | DWE03_num==8) ~ 1 , TRUE ~ 0)  ) %>%
  #improved wall: cement,stone,bricks,cement blocks, covered adobe, wood planks
  mutate(dwe04_cat=case_when( (DWE04_num==10| DWE04_num==11| DWE04_num==12|
                                 DWE04_num==13| DWE04_num==14| DWE04_num==15) ~ 1, TRUE ~ 0)  )
...
main <- main %>%
  ...
  mutate(unemployed=case_when(
      employed==0 & UNEM09_num==1 & UNEM10_num==1 ~ 1,  TRUE ~ 0)) %>%

  mutate(labour_force=case_when( employed==1 | unemployed==1 ~ 1) ) %>%

  mutate(outcome13_3=unemployed/labour_force)

table(main$employed, useNA = "ifany")
table(main$unemployed, useNA = "ifany")
table(main$labour_force, useNA = "ifany")
