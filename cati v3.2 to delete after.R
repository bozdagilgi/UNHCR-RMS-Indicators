...
###It's not recommended to clear your work space
...
main <- main %>%
  mutate(shelter=case_when(
    dwe05_cat==1 & housing==1 & !(DWE01 %in% c("3", "4", "5", "6", "7", "8", "9", "96")) ~ 1,
    TRUE ~ 0
  ))
...
# Check by population groups
ggplot(main, aes(x = factor(impact2_2), fill = pop_groups)) +
  geom_bar(position = "dodge", color = "black") +
  labs(title = "Distribution of Impact Indicator 2.2 by Population Groups",
       x = "Impact Indicator 2.2",
       y = "Count",
...
table_impact2_3 <- table(main$impact2_3, main$pop_groups)
...
main$EDU01 <- labelled_chr2dbl(main$EDU01)
main$EDU02 <- labelled_chr2dbl(main$EDU02)
main$EDU03 <- labelled_chr2dbl(main$EDU03)
...
table(main$impact3_2a)

ggplot(main, aes(x = factor(impact3_2a), fill = factor(impact3_2a))) +
  geom_bar(stat = "count", position = "stack", show.legend = FALSE) +
...
ggplot(main, aes(x = factor(impact3_2b), fill = factor(impact3_2a))) +
  geom_bar(stat = "count", position = "stack", show.legend = FALSE) +
...
main$REG03 <- labelled_chr2dbl(main$REG03) # birth certificate
main$REG04 <- labelled_chr2dbl(main$REG04) # birth has been registered
...
#count
table(main$outcome1_2)

##Percentages

table_outcome1_2 <- table(main$outcome1_2, main$HH04)
...
ggplot(main, aes(x = factor(outcome1_2), fill = factor(outcome1_2))) +
  geom_bar(stat = "count", position = "stack", show.legend = FALSE) +
...
main$REG05a <- labelled_chr2dbl(main$REG05a) # passport
main$REG05b <- labelled_chr2dbl(main$REG05b) # civil/government issued ID
main$REG05c <- labelled_chr2dbl(main$REG05c) # residency permit
main$REG05d <- labelled_chr2dbl(main$REG05d) # statelessness documentation
main$REG05e <- labelled_chr2dbl(main$REG05e) # household card of address/family book
main$REG05f <- labelled_chr2dbl(main$REG05f) # social security card
main$REG06 <- labelled_chr2dbl(main$REG06) # any other document establishes identity
...
main$REG01a <- labelled_chr2dbl(main$REG01a) # passport
main$REG01b <- labelled_chr2dbl(main$REG01b) # birth certificate
main$REG01c <- labelled_chr2dbl(main$REG01c) # civil/ government issued ID
main$REG01d <- labelled_chr2dbl(main$REG01d) # residency permit
main$REG01e <- labelled_chr2dbl(main$REG01e) # statelessness documentation
main$REG01f <- labelled_chr2dbl(main$REG01f) # household card of address/family book
main$REG01g <- labelled_chr2dbl(main$REG01g) # social security card
main$REG02 <- labelled_chr2dbl(main$REG02) # any other document establishes identity
...
#count
table(main$outcome1_3)

##Percentages

table_outcome1_3 <- table(main$outcome1_3, main$HH04)
...
ggplot(main, aes(x = factor(outcome1_3), fill = factor(outcome1_3))) +
  geom_bar(stat = "count", position = "stack", show.legend = FALSE) +
...
##descriptives
#count
table(main$outcome5_2)

##Percentages

table_outcome5_2 <- table(main$outcome5_2, main$HH04)
...
ggplot(main, aes(x = factor(outcome5_2), fill = factor(outcome5_2))) +
  geom_bar(stat = "count", position = "stack", show.legend = FALSE) +
...
#Turn into numeric
main$MMR03 <- labelled_chr2dbl(main$MMR03)
...
table_outcome10_1 <- table(main$outcome10_1, main$HH04)
...
ggplot(main, aes(x = factor(outcome10_1), fill = factor(outcome10_1))) +
  geom_bar(stat = "count", position = "stack", show.legend = FALSE) +
...
main <- main %>%
  ...
  mutate(outcome13_3 = factor(outcome13_3,
         ...
         table(main$outcome14_1)
         
         ##Percentages
         
         table_outcome14_1 <- table(main$outcome14_1)
